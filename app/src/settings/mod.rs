...
impl SettingsFileError {
    ...
    pub fn heading_and_description(&self, ctx: &AppContext) -> (String, String) {
        match self {
            Self::FileParseFailed(_) => (
                crate::localization::text("settings-file-error-heading", ctx),
                crate::localization::text("settings-file-parse-description", ctx),
            ),
            Self::InvalidSettings(keys) => match keys.len() {
                1 => {
                    let mut args = fluent_bundle::FluentArgs::new();
                    args.set("key", keys[0].as_str());
                    (
                        crate::localization::text("settings-file-error-heading", ctx),
                        crate::localization::text_with_args(
                            "settings-file-invalid-description",
                            &args,
                            ctx,
                        ),
                    )
                ...
                _ => {
                    let mut args = fluent_bundle::FluentArgs::new();
                    args.set("keys", keys.join(", "));
                    (
                        crate::localization::text("settings-file-errors-heading", ctx),
                        crate::localization::text_with_args(
                            "settings-file-invalid-many-description",
                            &args,
                            ctx,
                        ),
                    )
