...
fn load_default_ui_font_family(ctx: &mut AppContext) -> anyhow::Result<FamilyId> {
    warpui::fonts::Cache::handle(ctx).update(ctx, |font_cache, _| {
        ...
        // Prefer the Windows UI font that includes Simplified Chinese glyphs. This keeps localized
        // interface text available immediately, without waiting for the network-backed fallback
        // font loader. Systems without this optional Windows font then try Segoe UI and Roboto.
        #[cfg(windows)]
        if let Ok(font_family_id) = font_cache.load_system_font("Microsoft YaHei UI") {
